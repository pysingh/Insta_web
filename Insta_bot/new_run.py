...
class Main:
    def __init__(self):
        ...
        self.follow_timer = time.time() + (random.randint(-1 * 60, 1 * 60)) + (3600 / config["max_follows_per_hour"])
        self.like_timer = time.time() + (random.randint(-1 * 60, 1 * 60)) + (3600 / config["max_likes_per_hour"])
        self.max_likes = config["max_likes_per_hour"]
        self.max_follows = config["max_follows"]
        self.max_follows_per_hour = config["max_follows_per_hour"]
        self.max_messages = config["max_messages"]
        self.message_text = config["message_text"]
        self.message_link = config["message_link"]
        ...
        try:
            with open("usernames.txt", "r") as name_file:
                for line in name_file:
                    self.usernames.append(line.strip())
        except:
        ...
        try:
            with open("liked_media.txt", "r") as liked_file:
                for line in liked_file:
                    self.liked_media.append(int(line.strip()))
        except:
        ...
        try:
            with open("messaged.txt", "r") as messaged_file:
                for line in messaged_file:
                    self.messaged.append(line.strip())
        except:
    ...
    def start_bot(self):
        ...
        bot = Apollo(login=self.username, password=self.password)
        ...
        try:
            self.bot_follows = pickle.load(open("followed.dump", "rb"))
        except:
            pass

        try:
            self.day_timer = pickle.load(open("timers.dump", "rb"))
        except:
        ...
        bot.write_log("Users followed by bot: %i\n" % (len(self.bot_follows)))
        non_follows = [user for user in self.followed if user not in self.followers]
        bot.write_log("Users that do not follow back: %i\n" % (len(non_follows)))


def main():
    # Initialize variables
    global message_pause
    global cleanup
    global pause_follow
    global pause_like
    usernames = []
    ...
    follow_timer = time.time() + (random.randint(-1 * 60, 1 * 60)) + (3600 / config["max_follows_per_hour"])
    like_timer = time.time() + (random.randint(-1 * 60, 1 * 60)) + (3600 / config["max_likes_per_hour"])
    max_likes = config["max_likes_per_hour"]
    max_follows = config["max_follows"]
    max_follows_per_hour = config["max_follows_per_hour"]
    max_messages = config["max_messages"]
    message_text = config["message_text"]
    message_link = config["message_link"]
    message_counter = 0
    # message_string = config["message"]
    ...
    while True:
            ...
            if len(new_followers) > 0 and not message_pause:
                for user in new_followers:
                    ...
                    if message_text != "" and message_counter < max_messages and user[0] not in messaged:
                        if message_link != "":
                            sendMessage(bot, messaged, user, message_text, message_link)
                            message_counter += 1
                        else:
                            sendMessage(bot, messaged, user, message_text)
                            message_counter += 1

        # Follow loop
        if time.time() > follow_timer and not pause_follow:
            follow_timer = time.time() + (random.randint(-1 * 60, 1 * 60)) + (3600 / config["max_follows_per_hour"])
        ...
        # Like loop
        if time.time() > like_timer and not pause_like:
            like_timer = time.time() + (random.randint(-1 * 60, 1 * 60)) + (3600 / config["max_likes_per_hour"])
            if max_like_counter < max_likes:
                like_protocol(bot, liked_media, likers)
                max_like_counter += 1

        time.sleep(0.8)


def target_follows(username, password):
    with open("config.json") as confile:
        config = json.load(confile)
...
with pool:
    result1 = pool.apply_async(fun1, ('Nishaf', 'Naeem', ...))
